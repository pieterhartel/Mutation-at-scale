...
* The column `run` indicates the killing decision, which can be `Limit`, `TxEvMeth`, or `TxEvMethLimit`.
* The column `maxTx` indicates the number of the current transaction, where Tx 0 is the deployment transaction.
* The column `name` is the name of the contract (disambiguated).
* The column `address` is the address of the contract on the Ethereum blockchain.
* The column `SLOC` is the number of source lines of the contract.
* The column `transactions` indicates how many transactions were made against the contract on 1 Jan 2019.
* The column `ether` is the amount of Ether stored in the contract on 1 Jan 2019.
* The column `version` is the compiler version (from the pragma in the course code).
* The column `registration_date` is the date at which the contract was registered as a verified smart contract on Etherscan.
* The column `methodCnt` is the number of different pure methods in the source code of the mutant.
* The column `instructionCnt` is the number of instructions in the byte code of the compiled mutant.
* The column `executionCnt` is the number of bytecodes of the mutant that have been executed.
* The column `compFailCnt` is the number of still born mutants for the contract.
* The column `mutNotEqCnt` is the number of trivially non-equivalent mutants of the contract.
* The column `killCnt` is the number of mutants killed.

The data file `kill_detail_TxEvMethLimit.csv` contains many columns, of which the following are most relevant:
* Column `mutant` indicates the mutant number, and column `operator` indicates which mutation operator was applied.
* Column `status` indicates whether the test was killed by the mutant before the manual analysis.
...
* Column `mutant` indicates the mutant number, and column `operator` indicates which mutation operator was applied.
* Column `status` indicates whether the test was killed by the mutant before the manual analysis.
* Column `notes` indicates what needs to be done to kill the mutant and column `details` indicates how the test should be updated.
Note that we have not actually updated any tests.
