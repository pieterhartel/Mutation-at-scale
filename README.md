...
# 6 Basic analysis
Please run the followiwing script to perform the basic analysis:
```
$ bash diff_loop.sh
```
...
The first part of the `.diff` file for mutant 25 shows the textual change of the mutant.
...
This does not mean that the mutant is also killed by the gas limit criterion.
The calculation is done seperately from the basic analysis of differences reported here. 
There are no differences in the time stamps of the blocks (see the line `txTime`).
Even if there are differences, this does not mean that the transcations in the mutant and the original execute at different times.
...
## 6.2 Anlysis scripts
There are several analysis scripts not included in the replication package because we were unable to make them sufficiently robust in the time we had available.
Instead we have included the outputs of the scripts in the folder.
